...
public void paraCada(Consumer<Campo>funcao) {
	...
	public void abrir(int linha,int coluna) {
			...
			campos.stream().filter(x->x.getLinha()==linha&&x.getColuna()==coluna).findFirst().ifPresent(x->x.Abrir());
	...
	public void Marcar(int linha,int coluna) {
		campos.stream().filter(x->x.getLinha()==linha&&x.getColuna()==coluna).findFirst().ifPresent(x->x.AlternarMarcacao());
	}
	...
	private void mostrarMinas() {
		campos.stream().filter(x->x.isMinado()).filter(x->!x.isMarcado()).forEach(x->x.setAberto(true));
	}
